        ...
        <section className="group flex flex-col overflow-hidden rounded-lg border border-slate-200 bg-white transition hover:shadow-lg" >
            ...
            <div className="flex flex-1 flex-col p-5">
                <div className="mb-2 flex items-center gap-2 text-[10px] font-medium text-slate-400">
                    <time dateTime={data.created_date}>{data.created_date}</time>
                </div>
                <h3 className="mb-3 line-clamp-1 text-base font-bold text-slate-900 group-hover:text-blue-600">
                    <Link href={`/news/documents/${data.slug}`}>{data.title}</Link>
                </h3>
                <p className="mb-4 line-clamp-2 text-sm text-slate-500 font-normal! leading-relaxed" dangerouslySetInnerHTML={{
                    __html: data?.content || ""
                }}></p>
